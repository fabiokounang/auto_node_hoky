module.exports = (data) => {
  let query = `SELECT id_emoney, nama_emoney, alias_emoney, status_emoney FROM master_emoney`;
  ...
  page = (page - 1) * limit;
  if (['created_at', 'nama_emoney', 'alias_emoney'].includes(sort_attr)) sort_attr = data.sort_attr ? data.sort_attr : sort_attr;
  sort = data.sort ? data.sort == 1 ? 'ASC' : 'DESC' : sort;
  search = data.search ? data.search : search;

  if (search) query += ` WHERE nama_emoney LIKE '%${search}%' OR alias_emoney LIKE '%${search}%'`;
